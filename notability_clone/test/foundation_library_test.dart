...
void main() {
  group('Issue #1 — Foundation Library & App Launch QA Gate', () {
    testWidgets('Test 1 — App Launch: App launches without crashing and displays Library header', (WidgetTester tester) async {
    ...
    testWidgets('Test 2 — Empty Library: Displays clear empty state view when no notes exist', (WidgetTester tester) async {
      await tester.pumpWidget(const PaperNoteApp());
      await tester.pumpAndSettle();

      expect(find.text('No Notes Yet'), findsOneWidget);
      expect(find.text('Create your first notebook to start writing, drawing, and organizing your thoughts.'), findsOneWidget);
    ...
    testWidgets('Test 3 — Create New Note: Tapping + opens Note Editor route', (WidgetTester tester) async {
      await tester.pumpWidget(const PaperNoteApp());
      await tester.pumpAndSettle();
      ...
      expect(find.byKey(const Key('submit_create_note_button')), findsOneWidget);
    });

    testWidgets('Test 4 — Library Display: Newly created note appears immediately in the library list', (WidgetTester tester) async {
      await tester.pumpWidget(const PaperNoteApp());
      await tester.pumpAndSettle();

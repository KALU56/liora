...
void main() {
  group('Issue #4 — Touch Input, Handwriting Engine & Viewport Controls QA Gate', () {
    testWidgets('Test 15 — Canvas Loading: Canvas fills viewport without clipping', (WidgetTester tester) async {
      ...
      expect(find.byKey(const Key('note_interactive_viewer')), findsOneWidget);
    });

    testWidgets('Test 16 — Viewport Control Zoom In: Zooming in transforms viewport scale', (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(
        home: NoteEditorScreen(
          title: 'Zoom In Test',
        ),
      ...
      final Matrix4 initialMatrix = initialViewer.transformationController?.value ?? Matrix4.identity();
      ...
      final Matrix4 updatedMatrix = updatedViewer.transformationController?.value ?? Matrix4.identity();

      expect(updatedMatrix.getMaxScaleOnAxis(), greaterThan(initialMatrix.getMaxScaleOnAxis()));
    });

    testWidgets('Test 17 — Viewport Control Zoom Out: Zooming out scales canvas viewport accurately', (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(
        home: NoteEditorScreen(
          title: 'Zoom Out Test',
        ),
    ...
    testWidgets('Test 18 — Viewport Control Pan: Panning translates viewport without adding stray strokes in Pan Mode', (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(
        home: NoteEditorScreen(
          title: 'Pan Test',
        ),
      ...
      final canvasCenter = tester.getCenter(find.byKey(const Key('paper_canvas')));
      ...
      final HandwritingCanvasWidget canvasWidget = tester.widget(find.byKey(const Key('handwriting_canvas')));
      expect(canvasWidget.strokes, isEmpty);
    });

    testWidgets('Test 19 — Reset Zoom: Returns canvas scale to identity matrix (1.0)', (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(
        home: NoteEditorScreen(
      ...
      final Matrix4 resetMatrix = resetViewer.transformationController?.value ?? Matrix4.identity();

      expect(resetMatrix.getMaxScaleOnAxis(), equals(1.0));
    });

    testWidgets('Test 20 — Finger Writing: Touch down + move + up generates a valid stroke with accurate points', (WidgetTester tester) async {
      ...
      final center = tester.getCenter(find.byKey(const Key('handwriting_touch_listener')));

      final gesture = await tester.startGesture(center, pointer: 1, kind: PointerDeviceKind.touch);
    ...
    testWidgets('Test 21 — Capacitive Pen: Stylus input generates strokes predictably', (WidgetTester tester) async {
      List<Stroke> capturedStrokes = [];
      ...
      final gesture = await tester.startGesture(center, pointer: 2, kind: PointerDeviceKind.stylus);
    ...
    testWidgets('Test 22 — Stroke Independence: Separate drag gestures create independent stroke objects without joining', (WidgetTester tester) async {
      List<Stroke> capturedStrokes = [];
    ...
    testWidgets('Test 23 — Smooth Curve Pathing: Stroke converts points into quadratic bezier path', (WidgetTester tester) async {
    ...
    testWidgets('Test 24 — Undo Stroke: Tapping undo removes the most recent stroke', (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(
        home: NoteEditorScreen(
          title: 'Undo Test',
        ),
    ...
    testWidgets('Test 25 — Clear Canvas: Tapping clear removes all strokes', (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(
        home: NoteEditorScreen(
          title: 'Clear Test',
        ),
    ...
    testWidgets('Test 26 — TouchPoint Metadata: Preserves offset, pressure, and timestamp', (WidgetTester tester) async {
      final now = DateTime.now();
      ...
      expect(restored.timestamp.toIso8601String(), equals(now.toIso8601String()));
    });
  });
}
